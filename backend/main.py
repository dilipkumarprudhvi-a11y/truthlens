...
# spaCy is optional — graceful fallback if model not available
try:
    import spacy
    nlp = spacy.load("en_core_web_sm")
    NLP_AVAILABLE = True
except Exception as e:
    print(f"[WARNING] spaCy not available: {e}. NLP features will be disabled.")
    nlp = None
    NLP_AVAILABLE = False

app = FastAPI(title="TruthLens API v2")
...
def compute_sentiment(text):
    ...
    overall  = pos_count - neg_count - (fear_count * 0.5)
    if overall > 1:   tone = "Positive"
    elif overall < -1: tone = "Negative"
    else:              tone = "Neutral"
    return {"tone": tone, "positive_pct": pos_pct, "negative_pct": neg_pct, "fear_pct": fear_pct, "overall_score": round(overall, 2)}
...
def compute_bias(text):
    ...
    return {"leaning": leaning, "bias_score": bias_score, "left_triggers": left_hits, "right_triggers": right_hits, "amplifiers": amp_hits}

def compute_clickbait(text):
    clickbait_patterns = [
        r"you won'?t believe", r"shocking(ly)?", r"mind[\s-]?blow",
        r"will (make|leave) you", r"number \d+ will", r"what happens next",
        r"this is why", r"the truth about", r"they don'?t want",
        r"secret(s)? (revealed?|exposed?)", r"click (here|now)",
        r"must[\s-]?see", r"going viral", r"breaking:?", r"exclusive:?"
    ]
    text_lower = text.lower()
    triggered = []
    ...
    return {"score": score, "level": level, "triggers": triggered[:6], "caps_word_count": len(caps_words), "exclamation_count": exclamations, "question_count": questions}
...
def compute_writing_style(text, doc):
    ...
    return {"avg_word_length": avg_word_len, "passive_voice_count": passive_count, "quote_count": quote_count, "number_count": number_count, "url_count": url_count, "formality": formality, "sentence_count": len(sentences)}

def compute_virality_risk(fake_score, clickbait_score, sentiment):
    base  = (fake_score * 0.4) + (clickbait_score * 0.35) + (sentiment['negative_pct'] * 0.15) + (sentiment['fear_pct'] * 0.1)
...
def extract_claims(text, doc):
    ...
    for s in sentences:
        s = s.strip()
        if len(s) > 30:
            if any(ind in s.lower().split() for ind in claim_indicators):
                claims.append(s)
    return claims[:5]
...
def read_root():
    ...
    return HTMLResponse(content="<h1>TruthLens API is running. Frontend not found.</h1>")

# Serve frontend assets at root level for compatibility
@app.get("/style.css")
def get_css():
...
@app.get("/googleccc5aae6bf226ee5.html")
def get_google_verification():
    return Response(content="google-site-verification: googleccc5aae6bf226ee5.html\n", media_type="text/html")

@app.get("/robots.txt")
def get_robots():
    return Response(content="User-agent: *\nAllow: /\nSitemap: https://dilipkumarprudhvi-a11y.github.io/truthlens/sitemap.xml\n", media_type="text/plain")

@app.get("/health")
def health_check():
    return {"status": "ok", "nlp": NLP_AVAILABLE}
...
async def analyze_news(request: Request):
    ...
    exclamations = text.count("!")
    if len(caps_words) > 3: fake_score += 15
    if exclamations > 2:    fake_score += 10
    ...
    if result == "REAL" and evidence_entities:
        actual_news_mock = f"Verified: Major news outlets confirm reporting related to '{evidence_entities[0]['text']}'. Narrative aligns with documented public records."
